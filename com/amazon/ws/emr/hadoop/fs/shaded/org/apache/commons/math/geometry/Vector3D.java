  ...
  implements Serializable
{
  ...
  public Vector3D orthogonal()
  {
    ...
    {
      double inverse = 1.0D / Math.sqrt(y * y + z * z);
      return new Vector3D(0.0D, inverse * z, -inverse * y);
    }
    if ((y >= -threshold) && (y <= threshold))
    {
      double inverse = 1.0D / Math.sqrt(x * x + z * z);
      return new Vector3D(-inverse * z, 0.0D, inverse * x);
    }
    double inverse = 1.0D / Math.sqrt(x * x + y * y);
  ...
  public static double angle(Vector3D v1, Vector3D v2)
  {
    ...
    if ((dot < -threshold) || (dot > threshold))
    {
      ...
      return 3.141592653589793D - Math.asin(v3.getNorm() / normProduct);
    }
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.geometry.Vector3D
