...
import com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.stat.StatUtils;
  ...
  implements TTest
{
  ...
  protected double homoscedasticT(double m1, double m2, double v1, double v2, double n1, double n2)
  {
    ...
    return (m1 - m2) / Math.sqrt(pooledVariance * (1.0D / n1 + 1.0D / n2));
  }
    ...
    throws IllegalArgumentException
  {
    if ((data == null) || (data.length < 2)) {
      throw MathRuntimeException.createIllegalArgumentException("insufficient data for t statistic, needs at least 2, got {0}", new Object[] { Integer.valueOf(data == null ? 0 : data.length) });
    }
  }
    ...
    throws IllegalArgumentException
  {
    if ((stat == null) || (stat.getN() < 2L)) {
      throw MathRuntimeException.createIllegalArgumentException("insufficient data for t statistic, needs at least 2, got {0}", new Object[] { Long.valueOf(stat == null ? 0L : stat.getN()) });
    }
  }
}

/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.stat.inference.TTestImpl
