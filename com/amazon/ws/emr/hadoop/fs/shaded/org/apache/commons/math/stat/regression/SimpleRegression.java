  ...
  implements Serializable
{
  ...
  {
    return Math.max(0.0D, sumYY - sumXY * sumXY / sumXX);
  }
  ...
  {
    return Math.sqrt(getMeanSquareError() * (1.0D / n + xbar * xbar / sumXX));
  }
    ...
    throws MathException
  {
    if ((alpha >= 1.0D) || (alpha <= 0.0D)) {
      throw MathRuntimeException.createIllegalArgumentException("out of bounds significance level {0}, must be between {1} and {2}", new Object[] { Double.valueOf(alpha), Double.valueOf(0.0D), Double.valueOf(1.0D) });
    }
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.stat.regression.SimpleRegression
