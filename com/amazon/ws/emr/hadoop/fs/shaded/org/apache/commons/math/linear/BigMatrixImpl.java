  ...
  implements BigMatrix, Serializable
{
  ...
  public BigMatrixImpl(double[][] d)
  {
    ...
    for (int row = 1; row < nRows; row++) {
      if (d[row].length != nCols) {
        throw MathRuntimeException.createIllegalArgumentException("some rows have length {0} while others have length {1}", new Object[] { Integer.valueOf(nCols), Integer.valueOf(d[row].length) });
      }
    }
    copyIn(d);
    lu = ((BigDecimal[][])null);
    ...
    throws MatrixIndexException
  {
    ...
    catch (ArrayIndexOutOfBoundsException e)
    {
      ...
      arr$ = selectedColumns;len$ = arr$.length;i$ = 0;
    }
    for (; i$ < len$; i$++)
    {
      int column = arr$[i$];
    ...
    throws IllegalArgumentException
  {
    if (v.length != getColumnDimension()) {
      throw MathRuntimeException.createIllegalArgumentException("vector length mismatch: got {0} but expected {1}", new Object[] { Integer.valueOf(v.length), Integer.valueOf(getColumnDimension()) });
    }
    int nRows = getRowDimension();
    int nCols = getColumnDimension();
    BigDecimal[] out = new BigDecimal[nRows];
    ...
    throws IllegalArgumentException, InvalidMatrixException
  {
    int nRows = getRowDimension();
    if (b.length != nRows) {
      throw MathRuntimeException.createIllegalArgumentException("vector length mismatch: got {0} but expected {1}", new Object[] { Integer.valueOf(b.length), Integer.valueOf(nRows) });
    }
    ...
    throws IllegalArgumentException, InvalidMatrixException
  {
    if (b.getRowDimension() != getRowDimension()) {
      throw MathRuntimeException.createIllegalArgumentException("dimensions mismatch: got {0}x{1} but expected {2}x{3}", new Object[] { Integer.valueOf(b.getRowDimension()), Integer.valueOf(b.getColumnDimension()), Integer.valueOf(getRowDimension()), "n" });
    }
    if (!isSquare()) {
      throw new NonSquareMatrixException(getRowDimension(), getColumnDimension());
    }
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.linear.BigMatrixImpl
