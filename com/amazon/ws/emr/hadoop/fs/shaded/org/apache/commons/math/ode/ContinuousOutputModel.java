  ...
  implements StepHandler, Serializable
{
    ...
    throws DerivativeException
  {
    ...
    else
    {
      if (getInterpolatedState().length != model.getInterpolatedState().length) {
        throw MathRuntimeException.createIllegalArgumentException("dimension mismatch {0} != {1}", new Object[] { Integer.valueOf(getInterpolatedState().length), Integer.valueOf(model.getInterpolatedState().length) });
      }
      if ((forward ^ forward)) {
        throw MathRuntimeException.createIllegalArgumentException("propagation direction mismatch", new Object[0]);
      }
      ...
      if (Math.abs(gap) > 0.001D * Math.abs(step)) {
        throw MathRuntimeException.createIllegalArgumentException("{0} wide hole between models time ranges", new Object[] { Double.valueOf(Math.abs(gap)) });
      }
    }
  ...
  public void setInterpolatedTime(double time)
  {
    ...
    while (iMax - iMin > 5)
    {
      ...
      if ((Math.abs(tMed - tMin) < 1.0E-6D) || (Math.abs(tMax - tMed) < 1.0E-6D))
      {
        index = iMed;
      }
      else
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.ode.ContinuousOutputModel
