  ...
  implements EigenDecomposition
{
  ...
  private void findEigenVectors(double[][] householderMatrix)
  {
    ...
    if (maxAbsoluteValue != 0.0D) {
      for (int i = 0; i < n; i++)
      {
        if (Math.abs(realEigenvalues[i]) <= 1.1102230246251565E-16D * maxAbsoluteValue) {
          realEigenvalues[i] = 0.0D;
        }
        if (Math.abs(e[i]) <= 1.1102230246251565E-16D * maxAbsoluteValue) {
          e[i] = 0.0D;
        }
      }
    }
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.linear.EigenDecompositionImpl
