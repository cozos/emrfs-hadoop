  ...
  implements UnivariateRealFunction
{
    ...
    throws ArithmeticException
  {
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      if (d == 0.0D) {
        for (int k = 0; k < n; k++) {
          if ((i != k) && (x[i] == x[k])) {
            throw MathRuntimeException.createArithmeticException("identical abscissas x[{0}] == x[{1}] == {2} cause division by zero", new Object[] { Integer.valueOf(i), Integer.valueOf(k), Double.valueOf(x[i]) });
          }
        }
      }
      double t = y[i] / d;
    ...
    throws IllegalArgumentException
  {
    ...
    if (x.length < 2) {
      throw MathRuntimeException.createIllegalArgumentException("{0} points are required, got only {1}", new Object[] { Integer.valueOf(2), Integer.valueOf(x.length) });
    }
  }
}

/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.analysis.polynomials.PolynomialFunctionLagrangeForm
