  ...
  implements SingularValueDecomposition
{
  ...
  public RealMatrix getCovariance(double minSingularValue)
  {
    ...
    if (dimension == 0) {
      throw MathRuntimeException.createIllegalArgumentException("cutoff singular value is {0}, should be at most {1}", new Object[] { Double.valueOf(minSingularValue), Double.valueOf(singularValues[0]) });
    }
  ...
  {
    double threshold = Math.max(m, n) * Math.ulp(singularValues[0]);
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.linear.SingularValueDecompositionImpl
