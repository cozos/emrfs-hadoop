  ...
  extends AdaptiveStepsizeIntegrator
{
    ...
    throws DerivativeException
  {
    ...
    for (int j = 1; j < n; j++)
    {
      ...
      if ((performTest) && (j <= maxChecks) && (k < maxIter))
      {
        ...
        if (deltaNorm > 4.0D * Math.max(1.0E-15D, initialNorm)) {
          return false;
        }
      }
    }
    ...
    throws DerivativeException, IntegratorException
  {
    ...
    double log10R = Math.log(Math.max(1.0E-10D, tol)) / Math.log(10.0D);
    int targetIter = Math.max(1, Math.min(sequence.length - 2, (int)Math.floor(0.5D - 0.6D * log10R)));
    ...
    while (!lastStep)
    {
        ...
        else if (k > 0)
        {
          ...
          else
          {
            ...
            case 1: 
              ...
              break;
            default: 
              if (((firstTime) || (lastStep)) && (error <= 1.0D)) {
                loop = false;
              }
              break;
            }
      ...
      if ((denseOutput) && (!reject))
      {
        ...
        {
          int l2 = l / 2;
          double factor = Math.pow(0.5D * sequence[l2], l);
          ...
          {
            factor = Math.pow(0.5D * sequence[(j + l2)], l);
        ...
        if (mu >= 0)
        {
          ...
          if (useInterpolationError)
          {
            ...
            hInt = Math.abs(stepSize / Math.max(Math.pow(interpError, 1.0D / (mu + 4)), 0.01D));
          ...
          if (!reject)
          {
            ...
            {
              double dt = eventsHandlersManager.getEventTime() - stepStart;
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.ode.nonstiff.GraggBulirschStoerIntegrator
