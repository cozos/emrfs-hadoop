  ...
  implements RealTransformer
{
  ...
  public double[] transform2(double[] f)
    throws IllegalArgumentException
  {
    double scaling_coefficient = Math.sqrt(2.0D / (f.length - 1));
  ...
  {
    double[] data = FastFourierTransformer.sample(f, min, max, n);
    double scaling_coefficient = Math.sqrt(2.0D / (n - 1));
    ...
    throws IllegalArgumentException
  {
    ...
    if (!FastFourierTransformer.isPowerOf2(n)) {
      throw MathRuntimeException.createIllegalArgumentException("{0} is not a power of 2 plus one", new Object[] { Integer.valueOf(f.length) });
    }
    if (n == 1)
    {
    ...
    for (int i = 1; i < n >> 1; i++)
    {
      ...
      double b = Math.sin(i * 3.141592653589793D / n) * (f[i] - f[(n - i)]);
      double c = Math.cos(i * 3.141592653589793D / n) * (f[i] - f[(n - i)]);
      x[i] = (a - b);
      x[(n - i)] = (a + b);
      t1 += c;
    }
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.transform.FastCosineTransformer
