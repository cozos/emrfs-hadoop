  ...
  implements Serializable
{
    ...
    throws IllegalArgumentException
  {
    ...
    for (int iBlock = 0; iBlock < blockRows; iBlock++)
    {
      ...
      int pEnd = Math.min(pStart + 52, rows);
      int iHeight = pEnd - pStart;
      for (int jBlock = 0; jBlock < blockColumns; jBlock++)
      {
        int qStart = jBlock * 52;
        int qEnd = Math.min(qStart + 52, columns);
    ...
    throws IllegalArgumentException
  {
    ...
    catch (ClassCastException cce)
    {
      ...
      for (int iBlock = 0; iBlock < blockRows; iBlock++)
      {
        ...
        {
          int qStart = jBlock * 52;
          int qEnd = Math.min(qStart + 52, m.getColumnDimension());
  ...
  public BlockRealMatrix transpose()
  {
    ...
    for (int iBlock = 0; iBlock < blockColumns; iBlock++) {
      for (int jBlock = 0; jBlock < blockRows; jBlock++)
      {
        ...
        int pStart = iBlock * 52;
        int pEnd = Math.min(pStart + 52, columns);
        int qStart = jBlock * 52;
        int qEnd = Math.min(qStart + 52, rows);
        int k = 0;
        for (int p = pStart; p < pEnd; p++)
        {
    ...
    throws MatrixIndexException, MatrixVisitorException
  {
    ...
    for (int iBlock = startRow / 52; iBlock < 1 + endRow / 52; iBlock++)
    {
      ...
      int pEnd = Math.min((iBlock + 1) * 52, 1 + endRow);
      for (int p = pStart; p < pEnd; p++) {
        for (int jBlock = startColumn / 52; jBlock < 1 + endColumn / 52; jBlock++)
        {
          ...
          int qEnd = Math.min((jBlock + 1) * 52, 1 + endColumn);
          double[] block = blocks[(iBlock * blockColumns + jBlock)];
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.linear.BlockRealMatrix
