  ...
  implements FieldVector<T>, Serializable
{
  ...
  public ArrayFieldVector(T[] d, int pos, int size)
  {
    if (d.length < pos + size) {
      throw MathRuntimeException.createIllegalArgumentException("position {0} and size {1} don't fit to the size of the input array {2}", new Object[] { Integer.valueOf(pos), Integer.valueOf(size), Integer.valueOf(d.length) });
    }
    field = d[0].getField();
    data = buildArray(size);
  ...
  {
    if (data.length != n) {
      throw MathRuntimeException.createIllegalArgumentException("vector length mismatch: got {0} but expected {1}", new Object[] { Integer.valueOf(data.length), Integer.valueOf(n) });
    }
  }
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.linear.ArrayFieldVector
