  ...
  implements Serializable
{
    ...
    for (int iBlock = 0; iBlock < blockRows; iBlock++)
    {
      ...
      for (int jBlock = 0; jBlock < blockColumns; index++)
      {
        if (blockData[index].length != iHeight * blockWidth(jBlock)) {
          throw MathRuntimeException.createIllegalArgumentException("wrong array shape (block length = {0}, expected {1})", new Object[] { Integer.valueOf(blockData[index].length), Integer.valueOf(iHeight * blockWidth(jBlock)) });
        }
        if (copyArray) {
    ...
    throws IllegalArgumentException
  {
    ...
    for (int i = 0; i < rawData.length; i++)
    {
      ...
      if (length != columns) {
        throw MathRuntimeException.createIllegalArgumentException("some rows have length {0} while others have length {1}", new Object[] { Integer.valueOf(columns), Integer.valueOf(length) });
      }
    }
    ...
    for (int iBlock = 0; iBlock < blockRows; iBlock++)
    {
      ...
      int pEnd = Math.min(pStart + 36, rows);
      int iHeight = pEnd - pStart;
      for (int jBlock = 0; jBlock < blockColumns; jBlock++)
      {
        int qStart = jBlock * 36;
        int qEnd = Math.min(qStart + 36, columns);
    ...
    throws IllegalArgumentException
  {
    ...
    catch (ClassCastException cce)
    {
      ...
      for (int iBlock = 0; iBlock < blockRows; iBlock++)
      {
        ...
        {
          int qStart = jBlock * 36;
          int qEnd = Math.min(qStart + 36, m.getColumnDimension());
    ...
    throws MatrixIndexException
  {
    ...
    for (T[] subRow : subMatrix) {
      if (subRow.length != refLength) {
        throw MathRuntimeException.createIllegalArgumentException("some rows have length {0} while others have length {1}", new Object[] { Integer.valueOf(refLength), Integer.valueOf(subRow.length) });
      }
    }
  ...
  public FieldMatrix<T> transpose()
  {
    ...
    for (int iBlock = 0; iBlock < blockColumns; iBlock++) {
      for (int jBlock = 0; jBlock < blockRows; jBlock++)
      {
        ...
        int pStart = iBlock * 36;
        int pEnd = Math.min(pStart + 36, columns);
        int qStart = jBlock * 36;
        int qEnd = Math.min(qStart + 36, rows);
        int k = 0;
        for (int p = pStart; p < pEnd; p++)
        {
  ...
  {
    if (v.length != columns) {
      throw MathRuntimeException.createIllegalArgumentException("vector length mismatch: got {0} but expected {1}", new Object[] { Integer.valueOf(v.length), Integer.valueOf(columns) });
    }
    T[] out = buildArray(getField(), rows);
    T zero = (FieldElement)getField().getZero();
    for (int iBlock = 0; iBlock < blockRows; iBlock++)
  ...
  {
    if (v.length != rows) {
      throw MathRuntimeException.createIllegalArgumentException("vector length mismatch: got {0} but expected {1}", new Object[] { Integer.valueOf(v.length), Integer.valueOf(rows) });
    }
    T[] out = buildArray(getField(), columns);
    T zero = (FieldElement)getField().getZero();
    for (int jBlock = 0; jBlock < blockColumns; jBlock++)
    ...
    throws MatrixIndexException, MatrixVisitorException
  {
    ...
    for (int iBlock = startRow / 36; iBlock < 1 + endRow / 36; iBlock++)
    {
      ...
      int pEnd = Math.min((iBlock + 1) * 36, 1 + endRow);
      for (int p = pStart; p < pEnd; p++) {
        for (int jBlock = startColumn / 36; jBlock < 1 + endColumn / 36; jBlock++)
        {
          ...
          int qEnd = Math.min((jBlock + 1) * 36, 1 + endColumn);
          T[] block = blocks[(iBlock * blockColumns + jBlock)];
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.linear.BlockFieldMatrix
