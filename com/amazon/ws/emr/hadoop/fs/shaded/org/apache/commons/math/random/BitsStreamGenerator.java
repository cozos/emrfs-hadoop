  ...
  implements RandomGenerator
{
  ...
  public double nextGaussian()
  {
    ...
    if (Double.isNaN(nextGaussian))
    {
      ...
      double r = Math.sqrt(-2.0D * Math.log(y));
      double random = r * Math.cos(alpha);
  ...
  {
    if (n < 1) {
      throw MathRuntimeException.createIllegalArgumentException("upper bound must be positive ({0})", new Object[] { Integer.valueOf(n) });
    }
    int mask = n;
    mask |= mask >> 1;
    mask |= mask >> 2;
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.random.BitsStreamGenerator
