...
class BiDiagonalTransformer
{
  ...
  private void transformToUpperBiDiagonal()
  {
    ...
    for (int k = 0; k < n; k++)
    {
      ...
      double[] hK = householderVectors[k];
      double a = hK[k] > 0.0D ? -Math.sqrt(xNormSqr) : Math.sqrt(xNormSqr);
      main[k] = a;
      if (a != 0.0D)
      {
        hK[k] -= a;
      ...
      if (k < n - 1)
      {
        ...
        double b = hK[(k + 1)] > 0.0D ? -Math.sqrt(xNormSqr) : Math.sqrt(xNormSqr);
        secondary[k] = b;
        if (b != 0.0D)
        {
          hK[(k + 1)] -= b;
  ...
  private void transformToLowerBiDiagonal()
  {
    ...
    for (int k = 0; k < m; k++)
    {
      ...
      if (k < m - 1)
      {
        ...
        double b = hKp1[k] > 0.0D ? -Math.sqrt(xNormSqr) : Math.sqrt(xNormSqr);
        secondary[k] = b;
        if (b != 0.0D)
        {
          hKp1[k] -= b;
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.org.apache.commons.math.linear.BiDiagonalTransformer
