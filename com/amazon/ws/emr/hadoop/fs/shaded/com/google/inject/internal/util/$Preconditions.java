...
public final class $Preconditions
{
  ...
  public static void checkElementIndex(int index, int size, String desc)
  {
    ...
    if (index < 0) {
      throw new IndexOutOfBoundsException(format("%s (%s) must not be negative", new Object[] { desc, Integer.valueOf(index) }));
    }
    if (index >= size) {
      throw new IndexOutOfBoundsException(format("%s (%s) must be less than size (%s)", new Object[] { desc, Integer.valueOf(index), Integer.valueOf(size) }));
    }
  }
  ...
  public static void checkPositionIndex(int index, int size, String desc)
  {
    ...
    if (index > size) {
      throw new IndexOutOfBoundsException(format("%s (%s) must not be greater than size (%s)", new Object[] { desc, Integer.valueOf(index), Integer.valueOf(size) }));
    }
  }
  ...
  public static void checkPositionIndexes(int start, int end, int size)
  {
    ...
    if (end < start) {
      throw new IndexOutOfBoundsException(format("end index (%s) must not be less than start index (%s)", new Object[] { Integer.valueOf(end), Integer.valueOf(start) }));
    }
  }
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.com.google.inject.internal.util..Preconditions
