  ...
  implements Serializable
{
  ...
  private long aggregateBelowRange(Aggregate aggr, @Nullable AvlNode<E> node)
  {
    ...
    return aggr.treeAggregate(left) + aggr.nodeAggregate(node) + aggregateBelowRange(aggr, right);
  }
  ...
  private long aggregateAboveRange(Aggregate aggr, @Nullable AvlNode<E> node)
  {
    ...
    return aggr.treeAggregate(right) + aggr.nodeAggregate(node) + aggregateAboveRange(aggr, left);
  }
  ...
  {
    return new TreeMultiset(rootReference, range.intersect(GeneralRange.upTo(comparator(), upperBound, boundType)), header);
  }
  
  public SortedMultiset<E> tailMultiset(@Nullable E lowerBound, BoundType boundType)
  {
    return new TreeMultiset(rootReference, range.intersect(GeneralRange.downTo(comparator(), lowerBound, boundType)), header);
  }
  
  static int distinctElements(@Nullable AvlNode<?> node)
  {
    return node == null ? 0 : distinctElements;
  }
...
/* Location:
 * Qualified Name:     com.amazon.ws.emr.hadoop.fs.shaded.com.google.common.collect.TreeMultiset
